...
interface DataTableProps<T> {
    ...
    onServerRowsPerPageChange?: (rowsPerPage: number) => void;
}

export function DataTableWithPaginationInServer<T>({
    data = [],
    columns = [],
    actions = [],
    loading = false,
    ...
    const effectivePage = serverPagination && typeof serverPage === 'number' ? serverPage : page;
    const effectiveRowsPerPage =
        serverPagination && typeof serverRowsPerPage === 'number' ? serverRowsPerPage : rowsPerPage;
    const totalRows =
        serverPagination && typeof totalCount === 'number' ? totalCount : filteredData.length;
    ...
    const handleChangeRowsPerPage = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (serverPagination && onServerRowsPerPageChange) {
    ...
    if (vista === 1) {
        return (
            <Box sx={{ width: '100%' }} mt={MarginTop}>
                ...
                {showSummary && !loading && !error && totalRows > 0 && (
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                        Mostrando {effectivePage * effectiveRowsPerPage + 1}-{Math.min((effectivePage + 1) * effectiveRowsPerPage, totalRows)} de {totalRows} registros
                    </Typography>
                )}
            </Box>
        );
